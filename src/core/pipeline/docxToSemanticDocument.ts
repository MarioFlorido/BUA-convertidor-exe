...
export function resolveDocumentTitle(
  ...
  if (written && !isGeneratorBoilerplate(written)) {
    return { title: written, source: 'description' };
  }
  return {
...
/**
 * Textos que algunas herramientas dejan en «Comentarios» al generar el .docx.
 *
 * Elegimos Comentarios porque Word nunca lo rellena solo. Cierto dentro de
 * Word, falso fuera: la plantilla por defecto de python-docx trae
 * `<dc:description>generated by python-docx</dc:description>`, y un documento
 * hecho con esa librería —o copiado de uno que lo fuera— nace con Comentarios
 * puesto. El resultado era que TODOS esos documentos salían titulados
 * «generated by python-docx», en la portada del PDF y en la cabecera del ELPX.
 *
 * Coincidencia exacta, no por partes: lo que buscamos es el texto que la
 * herramienta escribe tal cual, y nadie titula así un documento de verdad.
 * Si aparece el de otra herramienta, se añade a esta lista.
 */
const GENERATOR_DESCRIPTIONS = [
  'generated by python-docx',
  // La nuestra: un Word salido del conversor de .elp antiguo trae esto puesto,
  // y ese Word está pensado para volver a entrar por aquí.
  ELP_CONVERSION_DESCRIPTION.toLowerCase(),
];

function isGeneratorBoilerplate(description: string): boolean {
  const normalized = description.toLowerCase().replace(/\s+/g, ' ').trim();
  return GENERATOR_DESCRIPTIONS.includes(normalized);
}
