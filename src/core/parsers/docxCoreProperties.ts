 ...
 * Comentarios no se rellena solo DENTRO de Word. Fuera sí: python-docx, por
 * ejemplo, deja «generated by python-docx» en su plantilla. Quien decide el
 * título (resolveDocumentTitle, en docxToSemanticDocument.ts) descarta esos
 * textos; este lector devuelve lo que hay en el fichero, sin interpretarlo.
 */

import { unzipSync, strFromU8 } from 'fflate';

/**
 * Lo que ConvertidoreXe deja en «Comentarios» al generar un Word desde un .elp
 * antiguo (ver elpToDocx.ts). Vive aquí, y no allí, porque hacen falta los dos
 * lados: se escribe al convertir .elp → Word y se DESCARTA al convertir ese
 * mismo Word → ELPX (ver resolveDocumentTitle). Sin descartarlo, el viaje de
 * ida y vuelta —que es justo para lo que está la herramienta— titulaba el
 * recurso con esta frase. Una sola constante para que no se desincronicen.
 */
export const ELP_CONVERSION_DESCRIPTION =
  'Convertido desde eXeLearning clásico (.elp) por ConvertidoreXe';
