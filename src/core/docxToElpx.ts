...
function generateContentXml(project: ImportedProject): string {
  ...
  const navStructuresXml = project.pages.map((page, index) => generateOdeNavStructureXml(page, index, pageIds)).join('');
...
function addPreviewHtmlEntries(entries: Record<string, Uint8Array>, project: ImportedProject): void {
  ...
  for (const [, page] of pages.entries()) {
    const html = generatePreviewPageHtml(project, pages, pages.indexOf(page));
    entries[page.href] = new TextEncoder().encode(html);
  }
}

function getPreviewPages(project: ImportedProject): Array<{
  title: string;
  href: string;
  pageNumber: number;
  level: 1 | 2 | 3 | 4;
...
function generatePreviewNavHtml(
  pages: Array<{ title: string; href: string; pageNumber: number; level: 1 | 2 | 3 | 4; parentIndex: number | null }>,
