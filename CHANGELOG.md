...
## Sin publicar — Julio-Septiembre 2026

### Los enlaces en negrita vuelven a verse como enlaces en el ELPX
- **Lo destapó** «Fuentes de información especializada para el TFG en Ciencias»: en una misma lista de recursos, unos nombres enlazados salían azules y otros gris oscuro, indistinguibles de una negrita normal. En Word todos se ven igual (estilo «Hipervínculo» más negrita).
- **La causa**: Word guarda un enlace de dos formas y Mammoth las anida al revés. Insertar vínculo (Ctrl+K) da `<a><strong>…</strong></a>`; un enlace pegado (código de campo HYPERLINK) da `<strong><a>…</a></strong>`. Los temas pintan `.exe-content strong` en `#2a2a2a` y el `base.css` de eXeLearning quita el subrayado a los enlaces, así que en la primera forma el gris del `strong` tapaba el color del enlace. En aquel documento había 57 del primer tipo (grises) y 7 del segundo (azules). Pasaba sobre todo en las viñetas solo porque es donde se pone en negrita el nombre del recurso.
- **Arreglo** (`src/core/utils/boldLinkColor.ts`): `.exe-content a strong,.exe-content a b{color:inherit}` en el CSS extra del ELPX y de la vista previa, junto al icono de los títulos enlazados. Vale para los nueve temas sin republicarlos y cubre también el enlace en negrita solo en parte (`<a><strong>Enlace</strong> (PDF)</a>`), que salía mitad gris, mitad azul. El PDF no lo necesitaba: la hoja de impresión no da color al `strong`.
- **Y un enlace roto en el mismo documento**: un enlace pegado con opciones, `HYPERLINK "https://www.ams.org/…/journals" \t "_blank"`, salía con `href="https://…/journals" \t "_blank"`. Mammoth 1.12 lee la URL con una expresión voraz que llega hasta la última comilla. `DocxParser` recorta ahora, dentro del propio Mammoth, todo lo que va desde la primera comilla (en `href` y en anclas `\l`): una URL o un marcador no pueden llevar comillas sin escapar. Afecta a `\t`, `\o` y cualquier otra opción.
- 6 tests nuevos (4 en `DocxParser.test.ts`, 2 en `ElpxRenderer.test.ts`). Color comprobado con el `base.css` y el `style.css` reales: las tres formas salen ahora en `#135d87`.

### El paso 2 recuerda cómo se configuró cada Word
- **El problema**: el equipo carga una y otra vez el mismo Word —o sus versiones en otros idiomas— y tenía que rehacer el árbol de contenido del paso 2 cada vez. Ahora, al pulsar «Continuar con esta estructura», la configuración se guarda en el navegador; al volver a cargar ese Word, el paso 2 se abre ya con ella. Nada que pulsar y ningún control nuevo: la única señal es que la frase de ayuda del paso 2 pasa a decir de dónde sale la estructura.
- **Qué cuenta como «el mismo Word»** (`src/core/services/structureMemory.ts`): misma secuencia de títulos H1/H2, sin distinguir mayúsculas, tildes ni espacios. **Una traducción** (otros títulos, pero mismo número de H1 y mismo número de H2 bajo cada uno, en el mismo orden) también recupera la configuración, y la frase lo dice. Cualquier otra cosa —una sección añadida, dos H2 fusionados— no coincide y el paso 2 sale en blanco, como siempre: aplicar una estructura desalineada en silencio sería peor que no aplicar nada.
- **Qué se guarda**: una huella de los títulos, la forma del documento y las elecciones (nivel de cada H1, opción de cada H2). Ni contenido ni imágenes: unos 170 bytes por documento, en `localStorage`, y solo los últimos 20 documentos. Una estructura dejada toda por defecto no se guarda (y olvida la anterior del mismo documento).
- **Alcance**: la memoria es la del navegador de cada persona, no viaja con el Word. El primer compañero que configura un documento en cada equipo lo hace una vez; los siguientes usos de ese documento y de sus traducciones en ese navegador ya salen hechos.
- Verificado en el navegador con `multipage.docx`: primera carga en blanco; recarga de la página y segunda carga con el árbol recuperado; una copia con los títulos traducidos recuperada como «similar»; un Word distinto en blanco.

### Auditoría de etiquetas: cinco fallos silenciosos más, y un validador que no se desincroniza
- **Lo destapó un aviso falso**: un `[vertical]` perfectamente colocado —solo en su párrafo, justo antes de la tabla— avisaba de que «no precede a una tabla» y, de haberse convertido, habría salido impreso como texto. La causa no estaba en el documento: `DocxParser` conserva la sangría manual de Word como `style` en línea (`<p style="margin-left:18pt">`) y los patrones de marcador exigían un `<p>` pelado, sin atributos. Basta con que el autor sangre el párrafo —o con que pegue contenido de una web, que arrastra sangrías propias— para que la etiqueta deje de reconocerse. Nuevo `P_OPEN_SRC` (un `<p>` que admite atributos) aplicado a los **siete sitios que compartían el fallo**: el validador de marcadores de tabla, `applyTableClasses`, las cajas de `applyDivClasses`, el aislamiento de marcadores de `normalizeSemanticMarkers`, los iframes embebidos y el `[fin acordeón]` de `SemanticBuilder`. Cuatro eran latentes: habrían fallado en silencio en cuanto una caja o un vídeo cayeran en un apartado con sangría.
- **Y obligó a auditar las cinco familias a fondo**, esta vez sin suponer qué HTML produce Word: se generó un `.docx` con todos los escenarios y se pasó por el parser real. Aparecieron cinco fallos más, todos con la misma forma —la conversión hace algo mal y el autor no se entera—, que es la razón de esta entrada: ninguno daba la cara.
- **Caja repartida entre bloques** (una `[definición]` en un ítem de lista y su `[fin]` en otro, o una en un `<p>` y otra en un `<li>`): el Caso B de `applyDivClasses` no sabía qué es una frontera de bloque y envolvía a caballo entre dos, `<li><div></li>…<li></div></li>`. El navegador lo deshacía y la caja salía **vacía**, sin un solo aviso. Ahora el contenido inline no puede cruzar un bloque (`BLOCK_BOUNDARY_SRC`) y, cuando la caja no se puede formar, avisa el validador (`box-not-formable`) con la indicación de qué colocación sí funciona.
- **Corchetes dentro de `<pre>`/`<code>`**: un ejemplo de código que contuviera `[importante]…[fin]` acababa envuelto en un `<div>` **dentro** del `<pre>` —HTML inválido y código alterado—, y además generaba avisos falsos. Ahora `protectingCode` aparca el contenido protegido mientras se transforma, con red de seguridad: si alguna marca se pierde por el camino se repite sin protección, así en el peor caso se vuelve a la conducta anterior pero nunca se pierde contenido. Importa para cualquier curso que transcriba código o prompts.
- **Marcador de recurso con un bookmark de Word dentro** (`[víd<a id="b"></a>eo:]`): `applyResourceLinks` descartaba el documento entero **antes** de normalizar, así que la etiqueta salía impresa y sin aviso. El descarte pasa a mirar el HTML ya normalizado.
- **`[fin acordeón]` en un ítem de lista** sigue sin partir el grupo, a propósito: hacerlo dejaría el `<ul>` de apertura en una mitad y el `</ul>` en la otra. Era el fallo más callado de todos, porque el marcador se limpia igual y no deja rastro; ahora lo avisa el validador (`accordion-marker`).
- **El validador ya no replica los patrones del transformador**: ejecuta `applyDivClasses` y mira qué marcadores sobreviven. Duplicar la regla en dos sitios es exactamente lo que desincronizó validación y conversión en el fallo de la sangría, y era cuestión de tiempo que volviera a pasar.
- 26 tests nuevos (11 en `HtmlTransformer.test.ts`, 14 en `semanticTagBalance.test.ts`, 1 en `buildFromStructure.test.ts`). Continúa el trabajo de «Etiquetado semántico — robustez y validación», más abajo.

### La ventana deja de congelarse durante la conversión
- **El problema no era la lentitud, era que la interfaz estaba muerta.** Las fases de la conversión están separadas por `await`, pero `await` sobre una promesa ya resuelta solo vacía la cola de MICROtareas: el navegador no pinta ni ejecuta temporizadores hasta agotar la pila. Los `onProgress` llamaban a `setState`, React encolaba el repintado y este no ocurría hasta que TODO había terminado. Medido con la interfaz real y un documento de 40 unidades con 120 imágenes: **la conversión entera era un único bloque de 1,3 s sin un solo respiro** (2,7 s en la primera, en frío).
- **Nuevo `src/core/utils/yieldToBrowser.ts`**, que encola una MACROtarea para que el navegador pueda pintar entre medias. Se llama en las fronteras de fase (`docxToSemanticDocument`, `semanticDocumentToElpx`, `ElpxRenderer.render`, `App.handleFileSelect`).
- **Usa `MessageChannel`, no `setTimeout(0)`.** La primera versión usaba `setTimeout` y la medición la tumbó: en pestañas de fondo Chrome limita los temporizadores a **uno por segundo**, así que cada cesión pasaba a costar un segundo entero y la conversión se iba de ~1,3 s a ~5 s — justo lo que le pasaría a quien deja un documento largo convirtiéndose y se va a otra pestaña. Los mensajes de `MessageChannel` no sufren ese recorte (es el mismo mecanismo, y por la misma razón, que usa el planificador de React). `requestAnimationFrame` tampoco vale: no se dispara en pestañas de fondo y la conversión quedaría detenida.
- **Las transformaciones de HTML se recorren pasada a pasada.** `applyAllTransforms` eran 8 pasadas encadenadas de un tirón: ~730 ms, el bloque más largo de toda la conversión. Ahora se exponen como `HTML_TRANSFORM_PASSES` y el pipeline las recorre cediendo entre cada una; ninguna llega a 170 ms por separado. `applyAllTransforms` se mantiene como función síncrona (la usan los tests y cualquier uso fuera del navegador) y el orden de las pasadas, que NO es intercambiable, queda fijado en la lista.
- **La extracción de imágenes cede página a página** (`ElpxRenderer.extractImagesToFiles`): decodificar base64 y hashear son ~240 ms seguidos que ahora se reparten.
- **Resultado medido**, tres pasadas por versión, misma interfaz y mismo documento: el bloqueo seguido más largo baja de **1299 ms (toda la conversión) a 180 ms** en caliente, y a 635 ms en la primera conversión en frío. Se pintan 8 mensajes de progreso donde antes no se pintaba ninguno hasta el final.
- **Coste asumido: la conversión tarda ~700 ms más** (de ~1,3 s a ~2,0 s en ese documento). Ese tiempo es lo que el navegador dedica a repintar de verdad, que antes se ahorraba porque no repintaba nada. Decisión consciente de Mario: prioridad a que no haya cortes prolongados; los milisegundos totales no son lo esencial.
- **Lo que sigue siendo un bloque indivisible es mammoth** (~480 ms en el peor caso medido): es una llamada interna a la librería y no se puede trocear desde fuera. Sacarlo a un Web Worker sí sería viable —mammoth no toca el DOM—, pero `DocxParser` anota ahí los tamaños reales de las imágenes y esa maquinaria se deja quieta a propósito.
...
### Acordeones y pestañas: `[fin-acordeón]` para cerrar el desplegable y seguir el apartado
- **El problema**: un Título 2 configurado como Acordeón o Pestañas se quedaba con *todo* el texto que le seguía hasta el próximo Título 2, así que los párrafos de cierre del apartado —una conclusión, una lista de lecturas— acababan atrapados dentro del último panel. No había forma de decir «el desplegable termina aquí».
- **Nuevo marcador `[fin-acordeón]`**, escrito en Word solo en su propio párrafo (`SemanticBuilder.splitAccordionEnd`): lo anterior queda como cuerpo del panel y lo posterior sale FUERA del `<div>` del acordeón, como flujo normal del apartado. Vale igual para pestañas. Tolerante a mayúsculas, tildes y guion/espacio (`[Fin acordeon]`, `[FIN-ACORDEON]`), y no lleva `[fin]`. Sin marcador, el comportamiento no cambia.
- **Donde no hay acordeón se elimina en silencio** (H2 como iDevice, como cabecera en texto, o fuera de todo H2): nunca sale impreso como texto literal.
- **Solo parte el grupo en un `<p>`**, a propósito: partir por un ítem de lista dejaría el `<ul>` de apertura en una mitad y el `</ul>` en la otra. Un marcador en una lista o compartiendo párrafo con texto no corta el acordeón —y como también se limpia, el fallo era invisible—, así que ahora lo avisa el validador antes de convertir (`semanticTagBalance`, nuevo `accordion-marker`). El párrafo sí puede llevar la sangría que Word guarda como `style`: exigir un `<p>` pelado hacía que el grupo no se partiera.
- Documentado en la guía de uso (Eje 1, Paso 3 y Eje 2, Paso 2), en el README y en `docs/README.html`.
